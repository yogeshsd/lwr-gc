 ...
 */
	function deleteUser(username){
		...
		if(x){
			var request = $.ajax({
				url: "/lwr/rest/users/remove/"+username,
				type: "POST",
	...
	function saveUser(){
		...
		if( role == ''){
			...
			users["role"]=role;
			var request = $.ajax({
				url: "/lwr/rest/users/save?displayname="+displayName+"&username="+username+"&password="+password+"&role="+role,
				type: "POST",
				success: function(resp){
							var row = $("#"+username);
							if( row.length == 0){
	...
	function saveProfile(){
		...
		var request = $.ajax({
			url: "/lwr/rest/users/save?displayname="+displayName+"&username="+username+"&password="+password+"&role="+role+"&charttype="+chartType+"&refreshInterval="+refreshInterval,
			type: "POST",
			success: function(resp){
						$("#savetext").html(resp);
					},
